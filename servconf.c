...
fmt_intarg(ServerOpCodes code, int val)
{
	...
	case sAddressFamily:
		...
		break;
	case sPermitRootLogin:
		multistate = multistate_permitrootlogin;
		mode = OSSH_RUST_FMT_INTARG_MULTISTATE;
		break;
	case sGatewayPorts:
		multistate = multistate_gatewayports;
		mode = OSSH_RUST_FMT_INTARG_MULTISTATE;
		break;
	case sCompression:
...
dump_cfg_int(ServerOpCodes code, int val)
{
	...
	if (code == sUnusedConnectionTimeout)
		mode = OSSH_RUST_CFG_INT_NONE_ZERO;
...
dump_cfg_oct(ServerOpCodes code, int val)
{
	...
	if (ossh_rust_cfg_int_parse(prefix, val, OSSH_RUST_CFG_INT_OCTAL,
	    &parsed) != 0)
		fatal_f("rust cfg int parse failed");
	buf = xmalloc(parsed.output_len + 1);
	if (ossh_rust_cfg_int_write(prefix, val, OSSH_RUST_CFG_INT_OCTAL,
...
{
#ifdef WITH_RUST_CRYPTO
	struct ossh_rust_strarray_lines_parse parsed;
	const char *prefix = lookup_opcode_name(code);
	char *buf;
#else
	u_int i;
...
#ifdef WITH_RUST_CRYPTO
	if (ossh_rust_strarray_lines_parse(prefix, (const char * const *)vals,
	    count, &parsed) != 0)
		fatal_f("rust strarray lines parse failed");
	if (!parsed.emit)
		return;
	buf = xmalloc(parsed.output_len + 1);
	if (ossh_rust_strarray_lines_write(prefix, (const char * const *)vals,
	    count, (u_char *)buf, parsed.output_len) != 0)
		fatal_f("rust strarray lines write failed");
	buf[parsed.output_len] = '\0';
	printf("%s", buf);
	free(buf);
#else
