 ...
 * so there is no second supported type available for a mismatch check. */
	...
	ASSERT_INT_EQ(sshkey_from_private(k1, &kcert), 0);
	ASSERT_PTR_NE(kcert, NULL);
	ASSERT_INT_EQ(sshkey_to_certified(kcert), 0);
	ASSERT_PTR_NE(kcert->cert, NULL);
	kcert->cert->type = SSH2_CERT_TYPE_USER;
	kcert->cert->serial = 4321;
	kcert->cert->key_id = strdup("agent-roundtrip");
	ASSERT_PTR_NE(kcert->cert->key_id, NULL);
	kcert->cert->principals = calloc(1, sizeof(*kcert->cert->principals));
	ASSERT_PTR_NE(kcert->cert->principals, NULL);
	kcert->cert->principals[0] = strdup("estragon");
	ASSERT_PTR_NE(kcert->cert->principals[0], NULL);
	kcert->cert->nprincipals = 1;
	kcert->cert->valid_after = 0;
	kcert->cert->valid_before = (uint64_t)-1;
	sshbuf_free(kcert->cert->critical);
	kcert->cert->critical = sshbuf_new();
	ASSERT_PTR_NE(kcert->cert->critical, NULL);
	sshbuf_free(kcert->cert->extensions);
	kcert->cert->extensions = sshbuf_new();
	ASSERT_PTR_NE(kcert->cert->extensions, NULL);
	put_opt(kcert->cert->extensions, "permit-agent-forwarding", NULL);
	ASSERT_INT_EQ(sshkey_from_private(k2, &kcert->cert->signature_key), 0);
