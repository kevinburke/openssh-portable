...
#[unsafe(no_mangle)]
pub extern "C" fn ossh_rust_sshkey_curve_nid_to_name(nid: c_int) -> *const c_char {
    match rust_sshkey_curve_nid_to_name(nid) {
...
#[unsafe(no_mangle)]
pub extern "C" fn ossh_rust_sshkey_ecdsa_bits_to_nid(bits: c_int, out: *mut c_int) -> c_int {
    if out.is_null() {
        return -1;
    }
    match rust_sshkey_ecdsa_bits_to_nid(bits) {
        Some(parsed) => {
            unsafe {
                *out = parsed;
            }
...
#[unsafe(no_mangle)]
pub extern "C" fn ossh_rust_sshkey_sigalg_match_plan(
    input: *const u8,
    input_len: usize,
    entries: *const *const RustSshkeyImplEntry,
    nentries: usize,
    out_match_kind: *mut c_int,
) -> c_int {
    if out_match_kind.is_null() {
        return -1;
    }
    match rust_sshkey_sigalg_match_plan(input, input_len, entries, nentries) {
        Some(kind) => {
            unsafe {
                *out_match_kind = kind;
            }
...
#[unsafe(no_mangle)]
pub extern "C" fn ossh_rust_sshkey_names_valid_include(
    input: *const u8,
    input_len: usize,
    allow_wildcard: c_int,
    plain_only: c_int,
    entries: *const *const RustSshkeyImplEntry,
    nentries: usize,
    out_valid: *mut c_int,
    ...
    match rust_sshkey_names_valid_include(
        input,
        input_len,
        allow_wildcard != 0,
        plain_only != 0,
...
#[unsafe(no_mangle)]
pub extern "C" fn ossh_rust_sshkey_private_deserialize_plan(
    input: *const u8,
    input_len: usize,
    entries: *const *const RustSshkeyImplEntry,
    nentries: usize,
    ...
    out_is_cert: *mut c_int,
    out_impl_index: *mut c_int,
    out_expected_cert_nid: *mut c_int,
) -> c_int {
    if out_type.is_null()
        || out_is_cert.is_null()
        || out_impl_index.is_null()
        || out_expected_cert_nid.is_null()
    {
        return -1;
    }
    match rust_sshkey_private_deserialize_plan(input, input_len, entries, nentries) {
        Some((type_, is_cert, impl_index, expected_cert_nid)) => {
            unsafe {
                ...
                *out_is_cert = c_int::from(is_cert);
                *out_impl_index = impl_index;
                *out_expected_cert_nid = expected_cert_nid;
            }
            0
        }
        None => -1,
