...
pub(crate) fn sshkey_curve_nid_to_name(nid: c_int) -> Option<&'static [u8]> {
    match nid {
        415 => Some(b"nistp256\0"),
        715 => Some(b"nistp384\0"),
        716 => Some(b"nistp521\0"),
        _ => None,
    }
}

pub(crate) fn sshkey_ecdsa_bits_to_nid(bits: c_int) -> Option<c_int> {
    match bits {
        256 => Some(415),
        384 => Some(715),
        521 => Some(716),
...
mod tests {
    ...
    #[test]
    fn ecdsa_bits_to_nid_matches_known_curves() {
        assert_eq!(sshkey_ecdsa_bits_to_nid(384), Some(715));
        assert_eq!(sshkey_ecdsa_bits_to_nid(-1), None);
    }
