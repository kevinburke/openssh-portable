...
mod tests {
    ...
    fn dh_roundtrip(group_id: c_int) {
        ...
        assert_eq!(
            0,
            dh_export_public(client, client_pub.as_mut_ptr(), client_pub.len())
        );
        assert_eq!(
            0,
            dh_export_public(server, server_pub.as_mut_ptr(), server_pub.len())
        );
    ...
    fn construct_group_from_params() {
        ...
        assert_eq!(
            0,
            dh_export_modulus(group, modulus.as_mut_ptr(), modulus.len())
        );
        assert_eq!(
            0,
